...
class CreatureService {
  ...
  checkData(p: { creature: Creature; base: BaseCreature; isAdjustment: boolean }): void {
    ...
    // An adjustment's own autoGenerate overrides the creature's per-key (shallow merge), so a
    // subset of files can fight/save as a different nominal level without that leaking into
    // sibling adjustments that don't declare their own.
    const adjustmentAutoGenerate = p.isAdjustment
      ? (p.base as CreatureAdjustment).autoGenerate
      : undefined;
    this.autogenerateData({
      data,
      autoGenerate: { ...p.creature.autoGenerate, ...adjustmentAutoGenerate },
      creature: p.creature,
  ...
  private autogenerateThac0(
    data: Partial<CreatureData>,
    parent?: CreatureData,
    options?: CreatureAutoGenerateThac0,
  ) {
    ...
    // An explicit override is a floor, not a blind replacement - never let it make THAC0 worse
    // than the creature's own real Hit Dice would already give (e.g. a stale override left behind
    // after level1 grew past it).
    if (options?.level) level = Math.max(level, options.level);
  ...
  private autogenerateSavingThrows({
    ...
    // An explicit override is a floor, not a blind replacement - never let it make saves worse
    // than the creature's own real Hit Dice would already give (e.g. a stale override left
    // behind after level1 grew past it - see the ogre mage chieftain / sirine discussion).
    const effectiveLevel =
      options?.level !== undefined ? Math.max(level.pnpValue, options.level) : level.pnpValue;
