...
Everything lands inside an `enhanced_creatures/` subfolder in the destination (e.g. `<BG1 path>/enhanced_creatures/enhanced_creatures.tp2`, `.../enhanced_creatures/lib/`, `.../enhanced_creatures/languages/`) — this matches where the tp2's own `%MOD_FOLDER%` references expect to find `lib/`/`languages/`, since `%MOD_FOLDER%` resolves to whichever folder the tp2 itself sits in. Run the tp2 from inside that subfolder (e.g. `weidu enhanced_creatures/enhanced_creatures.tp2` from the game root). This does not delete anything already in the destination — it only overwrites matching files inside `enhanced_creatures/`.
