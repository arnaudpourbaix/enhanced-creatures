...
**Outside-click, Escape, and scroll all dismiss regardless of pinned state.** The original design scoped outside-click/Escape to the pinned case only, reasoning that mouse-out already handles closing an unpinned popover. That held for a mouse, but not for keyboard users (WCAG 2.1 SC 1.4.13 requires hover-triggered content that obscures other content to be dismissible without moving the pointer — an unpinned popover had no such path) or for the touch edge case in the Non-goals section above (a stray hover-armed popover with no `click` to cancel it). Both listeners were widened to fire whenever the popover is open, pinned or not — harmless in the pinned case (unchanged), and now gives an unpinned popover the same dismiss paths as mouse-out. Scroll was already unconditional from the start, for the same underlying reason: the page can scroll without the mouse ever leaving the link.
...
- The outside-click / Escape / scroll listeners fire whenever the popover is open, regardless of `pinned` (see Behavior above) — no `pinned` gating on any of the three.
