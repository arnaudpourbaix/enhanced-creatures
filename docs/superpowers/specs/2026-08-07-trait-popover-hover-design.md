...
**Unpinned popovers ignore today's dismiss paths:** since mouse-out already handles closing an unpinned (hover-only) popover, the existing outside-click/Escape/scroll/close-button listeners only need to act when the popover is pinned. (The close button itself is only reachable by mouse in practice, and reaching it while a hover popover is open just means the pointer is inside the popover — which already cancels the close timer — so this is not a functional gap, just scoping the existing listeners to the pinned case so they don't fight with the hover close timer.)
